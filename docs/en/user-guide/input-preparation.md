...
=== "C-Phasing"
    ...
    C-Phasing can create the same source-contig-level COOL directly from either
    a `.pairs.pqs` dataset or a compressed `.pairs.gz` file. Following the
    official
    [C-Phasing `pairs2cool` workflow](https://wangyibin.github.io/CPhasing/latest/CLI/plot/),
    generate contig sizes and explicitly request the 1 kb resolution required
    by `cool2mcool`:
    ...
    For a standard compressed pairs file, replace `contacts.pairs.pqs` with
    `contacts.pairs.gz`. Set `--min-mapq` according to the mapping and filtering
    policy used for the library; add `--low-memory` when the normal conversion
    does not fit available memory.

    The FASTA, contig-size file, and pairs dataset must use the same
    source-contig names as AGP column 6. Stop at `pairs2cool` for the C-Studio
    main matrix. Do not use the chromosome-level `*.chrom.cool` generated by
    `cphasing plot -a` as the main input, because C-Studio performs the AGP
    projection itself.
...
=== "Cooler"
    ...
    actual input positions are 0-based. Decide MAPQ filtering before creating
    the COOL: aggregated counts cannot recover contacts that were discarded
    earlier.
